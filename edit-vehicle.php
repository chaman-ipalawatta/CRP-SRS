...
$vehicle = sql::getVehicle($_GET['id']);
$customerNameArr = sql::getAllCustomerNames();
$makeArr = sql::getAllMakes();
$oilTypesArr = sql::getOilTypes();
$makeId = sql::getMakeFromModel($vehicle['model_id']);
if(isset($_SESSION['postbackVals']['makes']))
{
    $modelArr = sql::getModelOfMake($_SESSION['postbackVals']['makes']);
}
else
{
    $modelArr = sql::getModelOfMake($makeId);
}
...
<div id="vehicleEdit">
    <div><h2>Edit Vehicle</h2></div>
    <form id="frmVehEdit" name="frmVehEdit" method="post"
      enctype="multipart/form-data" action="vehicle-edit-comp.php">
            ...
            <div class="rf_input"><select name="customerNames" id="customerNames">
                <?php
                    ...
                    <?php foreach ($array as $key=>$val): ?>
                        ...
                        <?php }else if ((!isset($postback['customerNames']))&&($key == $vehicle['customer_id'])){ ?>
                            <option value="<?php echo $key ?>" selected="select"><?php echo $val ?></option>
                        <?php }else{ ?>
        ...
        <div id="plate">
            ...
            <div class="rf_input"><input type="textbox" id="plateNo" name="plateNo"
                                         ...
                                         value="<?php echo (isset($postback['plateNo']))?
                                         $postback['plateNo']:$vehicle['plate_no']; ?>"/>
            <span id="description">Ex: 18-1234  /   WP-AA-1234  /   8-SRI-1234</span></div>
        </div>
        <div id="make">
            <div class="rf_label"><label>Make</label></div>
            <div class="rf_input">
                <select name="makes" id="makes" onchange="dochange('model',this.value)">
                ...
                <?php
                    ...
                    <?php foreach ($array as $key=>$val): ?>
                        <?php if ((isset($postback['makes']))&&($key == $postback['makes'])){ ?>
                            <option value="<?php echo $key ?>" selected="select"><?php echo $val ?></option>
                        <?php } else if((!isset($postback['makes']))&&($key == $makeId)){?>
                            <option value="<?php echo $key ?>" selected="select"><?php echo $val ?></option>
                        <?php } else { ?>
                            <option value="<?php echo $key ?>"><?php echo $val ?></option>
                        <?php } ?>
            ...
            <div class="rf_input">
                <select name="model" id="model">
                    ...
                    foreach ($modelArr as $array)
                    {
                        ...
                        {
                            if((isset($postback['model']))&&($key == $postback['model']))
                            {
                                echo '<option value="'. $key .'" selected="select">'.$val.'</option>';
                            }
                            else if((!isset($postback['model']))&&($key == $vehicle['model_id']))
                            {
                                echo '<option value="'. $key .'" selected="select">'.$val.'</option>';
                            }
                            else
                ...
                <?php
                    ...
                    <?php foreach ($array as $key=>$val): ?>
                            <?php if ((isset($postback['oilType']))&&($key == $postback['oilType'])){ ?>
                                <option value="<?php echo $key ?>" selected="select"><?php echo $val ?></option>
                            <?php }else if ((!isset($postback['oilType']))&&($key == $vehicle['comp_oil_type_id'])){ ?>
                                <option value="<?php echo $key ?>" selected="select"><?php echo $val ?></option>
                            <?php }else{ ?>
                                <option value="<?php echo $key ?>"><?php echo $val ?></option>
                            <?php } ?>
        ...
        <div id="notes">
                ...
                <div class="rf_input"><textarea rows="10" cols="50" id="notes" name="notes"
                        ...
                        echo (isset($postback['notes']))?trim($postback['notes']):$vehicle['notes'];
                        ?></textarea>
        ...
        <div id="regBtn">
                ...
                <div class="rf_input"><input type="submit" name="btnRegister" value="Update Vehicle">
                </div>
        </div>
    </form>
</div>
